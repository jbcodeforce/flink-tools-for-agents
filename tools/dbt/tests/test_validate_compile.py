...
from tools.dbt.flink_dbt_migrate.validate_compile import (
    DbtCompileError,
    compiled_model_path,
    find_dbt_project,
    read_compiled_sql,
...
CC_FLINK = (
    ...
    / "fixtures" / "flink-sql" / "10-windowing/tumble_then_hop_rolling/cc-flink"
)
...
def test_run_dbt_compile_invokes_subprocess(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "tools.dbt.flink_dbt_migrate.validate_compile.subprocess.run",
        fake_run,
    )
