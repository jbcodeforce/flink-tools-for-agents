...
from tools.dbt.flink_dbt_migrate.migrate import migrate_values_dml_to_seed
from tools.dbt.flink_dbt_migrate.migrate_dml_to_dbt import app, crawl_pipeline_folder
from tools.dbt.flink_dbt_migrate.flink_sql_processor import parse_ddl
from tools.dbt.flink_dbt_migrate.flink_sql_processor import is_values_insert, parse_values_dml
...
def test_parse_dml_still_rejects_values_statement() -> None:
    from tools.dbt.flink_dbt_migrate.flink_sql_processor import parse_dml
...
def test_cli_migrate_dry_run_routes_values_dml_to_seed(cli_runner: CliRunner, tmp_path: Path) -> None:
    ...
    result = cli_runner.invoke(app, ["migrate-one-file", str(dml_path), str(tmp_path / "seeds")])
...
def test_cli_migrate_write_creates_seed_files(cli_runner: CliRunner, tmp_path: Path) -> None:
    ...
    result = cli_runner.invoke(app, ["migrate-one-file", str(dml_path), str(seeds_dir), "--write"])
...
def test_cli_migrate_write_seed_requires_force_to_overwrite(cli_runner: CliRunner, tmp_path: Path) -> None:
    dml_path, _ = _write_seed_pair(tmp_path)
    seeds_dir = tmp_path / "seeds"
    cli_runner.invoke(app, ["migrate-one-file", str(dml_path), str(seeds_dir), "--write"])
...
def test_cli_migrate_seed_rejects_validate_flag(cli_runner: CliRunner, tmp_path: Path) -> None:
    ...
    result = cli_runner.invoke(
        app, ["migrate-one-file", str(dml_path), str(tmp_path / "seeds"), "--validate"]
    )

    # --validate is not a recognized flag; CLI rejects it (exit 1 or 2 depending on Typer version).
